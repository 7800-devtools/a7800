...
void menu_main::populate(float &customtop, float &custombottom)
{
		for (device_image_interface &image : image_interface_iterator(machine().root_device()))
	{
		if (image.user_loadable())
		{
			...
			item_append(_("Load ROM File"), "", 0, (void *)IMAGE_MENU_FILE_MANAGER);

			break;
		}
	}
	/* add input menu items */
	//item_append(_("Interface Options"), "", 0, (void *)INPUT_GROUPS);

	/* add slot info menu */
	if (slot_interface_iterator(machine().root_device()).first() != nullptr)
		item_append(_("Controller Selection"), "", 0, (void *)SLOT_DEVICES);

	//item_append(_("Input (this Machine)"), "", 0, (void *)INPUT_SPECIFIC);
	item_append(_("Console & Controller Inputs"), "", 0, (void *)INPUT_SPECIFIC);
	...
	if (ui().machine_info().has_dips())
		...
		item_append(_("Difficulty Switches"), "", 0, (void *)SETTINGS_DIP_SWITCHES);
	...
	item_append(_("Video & Audio Controls"), "", 0, (void *)SLIDERS);
	...
	/* add game info menu */
	item_append(_("Hardware & Information"), "", 0, (void *)GAME_INFO);
